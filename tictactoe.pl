...
nth_colonne(_, [], []).
nth_colonne(Index, [HC|TC], [HM|TM]) :-
nth1(Index, HM, HC),
nth_colonne(Index, TC, TM).
...
successeur(J, Etat, [L,C]) :- nth1(L, Etat, Ligne), nth1(C, Ligne, J).
...
% nb alignements potentiels réalisable à Situation - nb alignements possibles pour l'adversaires
findall(Ali, (alignement(Ali,Situation),possible(Ali, J)), CoupsJ),
length(CoupsJ, Nb),
adversaire(J, Ad),
findall(Ali, (alignement(Ali,Situation),possible(Ali, Ad)), CoupsAd),
length(CoupsAd, NbAd),
H is Nb-NbAd.
